...
class IndividualEval(object):
    ...
    def __init__(self, data):
        if data == 'mobile':       
            ...
            self.X, self.Y = get_gps('../data/porto/gps')
            self.max_locs = 10853
        self.max_distance = 0
    ...
    def get_periodicity(self, trajs):
        ...
        reps = []
        for traj in trajs:
            reps.append(float(len(set(traj)))/len(traj))
    ...
    def get_retention(self, data:np.ndarray)-> tuple[float, float]:
        flat = data.flatten()
        un, counts = np.unique(flat, return_counts=True)
        ret = 0
        for u in un:
            if u in range(self.max_locs):
                ret += 1
        
        ret = ret/self.max_locs
        check = len(un)/self.max_locs
        return ret, check
    
    def get_repetition(self, data:np.ndarray)->float:
        un =np.unique(data, axis=0)
        return len(data)-len(un)

    def get_individual_jsds(self, t1, t2, eval = False, length:int = 48):
        """
        ...
        du1_dist, _ = EvalUtils.arr_to_distribution(du1, 0, 1, length)
        du2_dist, _ = EvalUtils.arr_to_distribution(du2, 0, 1, length)
        du_jsd = EvalUtils.get_js_divergence(du1_dist, du2_dist)
        
        p1 = self.get_periodicity(t1)
        p2 = self.get_periodicity(t2)
        p1_dist, _ = EvalUtils.arr_to_distribution(p1, 0, 1, length)
        p2_dist, _ = EvalUtils.arr_to_distribution(p2, 0, 1, length)
        ...
        if eval:
            ret, check = self.get_retention(t2)
            assert ret == check
            return d_jsd,  g_jsd,  du_jsd,  p_jsd, l_jsd, f_jsd, ret
...
def evaluate(datasets, epsilon, comment=None):
    ...
    test_data = read_data_from_file(f'../data/{opt.datasets}/test.data')
    gene_data = read_data_from_file(f'../data/{opt.datasets}/{int(opt.epsilon)}/gene.data')

    d_jsd,  g_jsd,  du_jsd,  p_jsd, l_jsd, f_jsd, retention = individualEval.get_individual_jsds(test_data,gene_data, length)
    
    printer = f'{opt.datasets[0]},{epsilon},{d_jsd},{g_jsd},{du_jsd},{p_jsd},{l_jsd},{f_jsd},{retention},{opt.comment}\n'
...
if __name__ == "__main__":
    ...
    if opt.datasets == 'mobile':
        max_locs = 8606
    elif opt.datasets == 'geolife':
        max_locs = 23768
    elif opt.datasets == 'porto':
        max_locs = 10853
    evaluate(opt.datasets, opt.epsilon, opt.comment)
