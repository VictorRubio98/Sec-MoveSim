...
class GenDataset(Dataset):
    ...
    def __init__(self, data) -> None:
        ...
        self.indices = torch.LongTensor(range(len(self.data)))
...
class NewGenIter(DataLoader):
    ...
    def __init__(self, data_file, batch_size):
        ...
        self.indices = range(self.data_num)
        self.num_batches = int(
            math.ceil(float(self.data_num) / batch_size))
        self.idx = 0
        self.batch_size = batch_size
        self.dataset = GenDataset(self.data_lis)
        super(NewGenIter, self).__init__(dataset = self.dataset)
...
class DisDataset(Dataset):
    ...
    def __init__(self, data, labels) -> None:
        ...
        self.pairs = list(zip(data, self.labels))
...
class DisDataIter(DataLoader):
    ...
    def __init__(self, real_data_file, fake_data_file, batch_size,length):
        ...
        data = real_data_lis + fake_data_lis
        self.dataset = DisDataset(data , self.labels)
        self.pairs = list(zip(data, self.labels))
        self.data_num = len(self.pairs)
        self.indices = range(self.data_num)
        self.num_batches = int(math.ceil(float(self.data_num) / self.batch_size))
        self.idx = 0
        super(DisDataIter, self).__init__(dataset = self.dataset)
