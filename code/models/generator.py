...
class Generator(nn.Module):
    ...
    def sample(self, batch_size, seq_len, x=None):
        ...
        if flag:
            ...
            elif self.starting_sample == 'real' or self.starting_sample == 'infer':
        ...
        if flag and self.starting_sample == 'infer':
            #return noise and sampled output
            return output, x
        return output
...
class ATGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.attn = DPMultiheadAttention(self.hidden_dim,  4) if self.make_private else nn.MultiheadAttention(self.hidden_dim, 4)
        ...
        self.attn2 = DPMultiheadAttention(self.hidden_dim, 1) if self.make_private else nn.MultiheadAttention(self.hidden_dim, 4)
    ...
    def forward(self, x_l, x_t):
        ...
        mat1 = torch.Tensor(mat1).to(self.device).reshape(x_l.shape[0],x_l.shape[1],self.total_locations)
        mat2 = torch.Tensor(mat2).to(self.device).reshape(x_l.shape[0],x_l.shape[1],self.total_locations)
        ...
        mat1 = F.selu(self.linear_mat1(mat1))
        mat1 = torch.sigmoid(self.linear_mat1_2(mat1))
        mat1 = F.normalize(mat1)
        
        mat2 = F.selu(self.linear_mat2(mat2))
        ...
        if self.function:
            ...
            x = x.reshape(-1, self.total_locations)
            mat1 = mat1.reshape(-1, self.total_locations)
            mat2 = mat2.reshape(-1, self.total_locations)
    ...
    def sample(self,batch_size, seq_len, x=None, inference = False):
        """
        ...
        if flag:
            if self.starting_sample == 'zero':
                x = torch.LongTensor(torch.zeros((batch_size, 1)).type(torch.LongTensor)).to(self.device)
            elif self.starting_sample == 'rand':
